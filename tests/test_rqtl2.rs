...
mod tests {
  ...
  #[test]
  fn parsers() {
    use rqtl2::rqtl2::GenoParser;
    let mut f = create_test_file(
      "test_geno_parsers_1.txt",
      "#test file\n#comment\nmarker	10	12	38	39\nrs31443144	ABAH",
    )
    .expect("Failed file creation");
    let comments = rqtl2::rqtl2::parse_comments(&mut f).expect("Parsing failed");
    let markers = rqtl2::rqtl2::parse_markers(&mut f).expect("Parsing failed");
  ...
  fn read_snps() {
    ...
    let geno = rqtl2::rqtl2::parse_geno(&mut f, &hab_mapper).unwrap();
    ...
    let geno_from_struct = rqtl2::rqtl2::GenoParser::new_with_file(f, hab_mapper)
  ...
  fn snp_iter() {
    ...
    let mut geno_parser =
      rqtl2::rqtl2::GenoParser::new_with_file(f, hab_mapper).expect("Failed to create GenoParser");
  ...
  fn calc_kinsip() {
    ...
    let mut geno_parser = rqtl2::rqtl2::GenoParser::new_with_file(f, hab_mapper.clone())
      .expect("Failed to create GenoParser");

    let mut matr = geno_parser.calc_kinship_on_cpu(1).unwrap();
